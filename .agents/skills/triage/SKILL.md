...
5. **Apply the outcome:**
   ...
   Every outcome uses one state-replacement transition: re-read the item, remove every configured workflow-state label currently present, apply exactly the selected target label, then re-read and verify that the target is the only configured workflow-state label. Perform this transition for `needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`, and `wontfix`; closing a `wontfix` item happens only after the singleton label is verified. If removal, application, or verification is partial or ambiguous, stop without claiming the outcome and reconcile the observed tracker state before another mutation.
