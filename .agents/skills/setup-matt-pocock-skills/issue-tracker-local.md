...
- **Needs-info publication**: append the structured triage note with the exact questions, re-read and verify its content, then replace `Status:` with `needs-info`. Inventory retains any file already in `needs-info` without a verified question record as recoverable partial state in maintainer attention even when no reporter activity exists.
- **Wontfix terminal state**: Local Markdown's verified singleton `Status: wontfix` is itself the terminal operation, so there is no separate close that can leave an open `wontfix` recovery item. A custom file workflow with a separate close flag must inventory any partial terminal state until that flag is verified.
...
- **Publication recovery**: before any write, verify the durable source's repository-defined terminal planning state, then inventory the complete feature issue directory at `.scratch/<feature-slug>/issues/*.md`, reject malformed or duplicate numbers and keys, and compare the complete approved key set. Each ticket has `Ticket publication key: <source-identity>/<approved-breakdown-digest>/<approved-ordinal>`. Treat every unmatched key with the same source-identity prefix as a prior generation and require explicit verified reuse or supersession disposition before any create. Zero matches then permits creation at one unused number with an exclusive new path; one match resumes that exact file from its observed stage; multiple matches stop for explicit duplicate disposition. Never implicitly overwrite or replace a ticket, and wire `Blocked by` references only after the approved keys have been reconciled to actual local numbers.
...
- **Map publication recovery**: the map carries `Map publication key: <effort-id>/<normalized-destination-digest>`. Before writing `.scratch/<effort>/map.md`, reconcile all local map files by destination digest and exact full key. Zero active matches with no closed candidate permits one exclusive first-effort create; one active match resumes through body/state and child-publication reconciliation; multiple active or exact-key matches stop for explicit disposition. No active match with a closed candidate requires explicit resume-versus-new-effort disposition. To resume the closed candidate, replace its singleton `Status: resolved` with `Status: open`, re-read the same file, and verify exactly one `Status: open` before changing its body or children. For a new effort, generate a fresh stable effort ID before creation while retaining the destination digest. Never overwrite a path implicitly.
