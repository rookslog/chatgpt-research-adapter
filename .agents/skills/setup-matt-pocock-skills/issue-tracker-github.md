...
Every runnable ticket carries the complete contract defined by `.agents/skills/triage/AGENT-BRIEF.md`: closure target; start revision/evidence and blockers; owned write/output set and non-goals; deterministic verification; external/live boundary; review declaration; root integration; and route fit. A Wayfinder planning child records these fields under its body `## Execution contract`. A `ready-for-agent` implementation ticket uses the latest complete Agent Brief **comment** as its authoritative contract; create the issue body and relationships first, post and verify that comment, then apply the ready label. Missing fields require `Contract status: needs-clarification` and exclusion from the frontier.
