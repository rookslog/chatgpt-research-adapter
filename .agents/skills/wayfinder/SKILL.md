...
2. Choose the ticket. Whether the user named one or you select the first frontier ticket, verify that it is open, unblocked, unassigned, and carries any required execution contract. A named ticket does not bypass these gates. For delegated AFK work, root persists the pre-dispatch run record before assignment, verifies the assignment, dispatches once, and persists the task locator; failures use the delegated recovery rule. For HITL work, root assigns and verifies the live owner/root, then begins the exchange without a run record, dispatch, or locator.
