...
Every child headed for closure—selected, collected, or invalidated—enters one
root-owned closure queue. Each must remain open while root publishes and
verifies its answer or disposition, updates the map and dependencies, and
re-reads the affected frontier; only then may root close that child last. Never
close an invalidated sibling inline merely because its disposition is known.
...
4. **Reconcile while the ticket is still open.** Add newly surfaced tickets through the recoverable create-attach/index-verify transition, graduate any fog the answer has made specifiable, and clear each graduated patch from **Not yet specified**. Keep every invalidated ticket open and add it to the same root-owned closure queue; publish and verify its disposition, update the map and dependency graph, re-read the affected frontier, and close it last before continuing. If the answer places work beyond the destination, rule it out of scope rather than recording it as a route decision.
