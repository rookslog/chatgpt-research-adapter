...
Map publication is recoverable too. Derive `Map publication key: <normalized-destination-digest>` from the settled destination and persist it in the map body. Before permitting map creation, use the configured tracker operation to reconcile the complete open-and-closed map inventory: **zero matches** permits one create, **one match** resumes that exact map from its observed stage, and **multiple matches** stops for explicit duplicate disposition. A one-match recovery reconciles the map body, child publication, and current state; it never creates a replacement because identity retention or later child publication failed. A closed match or a materially redrawn destination requires explicit resume-versus-new-effort disposition.
...
```markdown
Map publication key: <normalized-destination-digest>
...
3. **Create or recover the map** (label `wayfinder:map`): compute the exact destination key and perform the tracker-defined zero/one/multiple reconciliation before any create. Create once only on zero matches. On one match, resume the observed map and verify its identity/state instead of replacing it. Then ensure Destination and Notes are filled in, Decisions-so-far is initially empty for a new map, and the fog is sketched into **Not yet specified**.
