...
Put the actual logic (the bit that's answering the question) in a single `<script>` block written as a small, pure module that can serve as precise prototype evidence. The page and module are both prototype artifacts, not production code; an authorized later implementation can rewrite the validated behavior into the real codebase.
...
Once the prototype has answered its question, capture the answer, then capture the prototype the way the [SKILL](SKILL.md) describes. The HTML shell rides along to the throwaway branch that keeps the prototype as a primary source and stays trivially re-runnable there. If production execution is explicitly authorized under the parent contract, rewrite the validated reducer / machine / function set into the real module; otherwise record the validated behavior and required promotion work on the planning ticket without modifying production code.
