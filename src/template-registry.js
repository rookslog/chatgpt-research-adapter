...
async function regularBytes(path) {
  ...
  try { const stat = await handle.stat(); if (!stat.isFile()) fail('template path is not regular', 'ERR_TEMPLATE_PATH'); return await handle.readFile(); }
...
export async function loadTemplate({ templatesRoot, templateId, version }) {
  ...
  const manifest = parseStrictJsonBuffer(await regularBytes(pathWithin(templateId, `${version}.json`)), { requireObjectRoot: true });
