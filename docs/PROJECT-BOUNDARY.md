...
The Deep follow-up surface is closed: `status --output-root <absolute> --job-id <id>` starts no provider process and creates no filesystem entry; it may `fsync` existing parent directories before accepting visible terminal records whose publication durability is otherwise uncertain. `collect --output-root <absolute> --job-id <id> --opencli <absolute>` uses only `deep-research-result --wait false`; and `wait` with the same required arguments uses only `--wait true`. These grammars accept neither a prompt nor mode and cannot submit. Status validates an existing completion event but does not create a missing event.

Accepted jobs are resumable. Collectors use immutable append-only generation owner/release receipts in `response/collector-locks/`; a failed release may add an immutable owner-hash-bound abandonment receipt so another local process can recover without waiting for the still-healthy wrapper process to exit. An atomically replaced, owner-hash-bound `collector-head.json` indexes the latest acquired generation so steady-state polls validate only the current terminal/successor tail instead of rescanning the complete journal; legacy journals receive one compatibility scan before their next acquisition establishes the index. Only a released, durably abandoned, or provably dead owner can be superseded, and a live owner is never time-stolen. Completion publishes staged, immutable artifacts in order: `response/report.md`, `response/result.json`, `response/result.committed.json`, then `response/events/research.completed.v1.json`; any completed collector return has first observed publisher release and finalized or validated the event. The terminal result permits the 256 KiB transport envelope plus 64 KiB of wrapper metadata. The event schema is `m006.research-completion-event.v1`; its type is `research.completed.v1`; its exact fields are `schema`, `type`, `job_id`, `turn_id`, `conversation_id`, `conversation_url`, `result_path`, `result_sha256`, `report_path`, `report_sha256`, `source_count`, and `completed_at`. A later non-submitting collection can repair an interrupted result commit or event publication; no callback, delivery, or provider submission is added.

Once a contiguous successor owner is durably present, it is the takeover authority and historical predecessor PID liveness is not re-evaluated; this avoids PID-reuse deadlock while preserving the live-owner check before successor creation. Direct wait options reject negative or non-integer termination grace before preflight so child shutdown cannot expand the absolute wait deadline.

The absolute deadline is threaded through OpenCLI identity preflight and into the private compatibility reader. Each child budget is recomputed immediately before spawn; an expired deadline prevents spawn, and a preflight or result whose required post-child work crosses the deadline is rejected as timed out. The private copy is still cleaned synchronously. Because Node's recursive filesystem promises are not cancelable, mandatory cleanup can delay promise settlement beyond the deadline; a strict wall-clock return bound would require a separate killable helper-process design.
