...
Standard submission is intentionally refused while the explicit-intent driver is being qualified. Offline preparation requires `model_family` and `effort` and writes a v2 receipt; recognized intent still does not enable sending. [Current behavior, compatibility and remaining work](docs/STANDARD-GROUNDWORK.md) supersede the historical Standard usability claims below. Web/Deep retain their implementation paths; this does not establish live qualification.
...
`[M007 DECISION — 2026-08-30]` The three current fail-closed compatibility families are enough to trigger an architecture review, but not an automatic fork. Continue the wrapper plus the existing temporary pinned-anchor overlays; do not add a fourth compatibility family without comparing that route against an integration-focused internal fork and an alternate controller. Runtime guards detect required anchor/caller drift, not arbitrary full-file changes; upgrades require complete-file diff and requalification. OpenCLI `v1.8.8` is now released, but the installed pin remains `v1.8.7`; its ChatGPT `utils.js` and `ask.js` blobs are unchanged in `v1.8.8` and current upstream `main`, while the complete newer package is not qualified. See [M007 dependency ownership](docs/research/M007-DEPENDENCY-OWNERSHIP.md).
...
The persistent Chrome profile must be running with the installed OpenCLI Browser Bridge enabled and connected. The default `standard` mode is live-verified. Submission uses the persistent container; response collection reloads only the returned conversation in an ephemeral reader and requests Markdown so a partially rendered submission DOM is not mistaken for the completed answer. Standard and Web retain their synchronous implementation paths. `[HISTORICAL LIVE OBSERVATION — 2026-08-24]` The then-current Web and Deep tool selectors failed before prompt submission; current live Deep completion extraction remains unqualified and is not implied by the deterministic lifecycle. Each successful standard call creates a job directory containing `response/answer.md` and `response/result.json`.

## Deep asynchronous lifecycle

For a prepared Deep job, `ask --mode deep` is the sole submission operation. It writes durable intent, invokes one pinned OpenCLI v1.8.7 `chatgpt ask`, validates and persists the conversation handoff, writes immutable `response/running.json`, and returns `running`; it does not read a result in that operation. An intent without a handoff is `attention_required` and is never resubmitted automatically.

The typed follow-up commands take no prompt or mode:

```text
chatgpt-research status --output-root <absolute> --job-id <id>
chatgpt-research collect --output-root <absolute> --job-id <id> --opencli <absolute>
chatgpt-research wait --output-root <absolute> --job-id <id> --opencli <absolute>
```

`status` starts no provider process and creates no filesystem entry. Before accepting already-visible terminal records, it may `fsync` their existing parent directories to repair uncertain publication durability; it validates a pre-existing completion event but does not create a missing one. `collect` invokes only `deep-research-result --wait false`; `wait` invokes only `deep-research-result --wait true`. Neither can submit a prompt. Accepted state remains resumable across restart.

Collectors serialize with immutable append-only generation receipts in `response/collector-locks/`; a later generation may supersede only a released or provably dead owner, never a live long waiter. On completion, the adapter publishes `response/report.md`, then immutable `response/result.json`, then `response/events/research.completed.v1.json`; a collector returns `completed` only after finalizing or validating that event. The event schema is `m006.research-completion-event.v1`, its type is `research.completed.v1`, and its exact fields are `schema`, `type`, `job_id`, `turn_id`, `conversation_id`, `conversation_url`, `result_path`, `result_sha256`, `report_path`, `report_sha256`, `source_count`, and `completed_at`. Interrupted event publication is recoverable by a later non-submitting collection; no callback or external delivery is performed.
...
The M003 slice established the first controlled mapping to pinned OpenCLI and proved the no-retry ambiguity contract. The owner subsequently authorized M004, persistent Chrome, and live execution. M004 changed the practical standard flow to submit without waiting and then read the specific returned conversation until its completed assistant response is available. That flow succeeded live and saved `CHATGPT_RESEARCH_PRODUCTION_OK`. A later Extra High answer exposed a stale persistent-reader DOM that contained only a table header; the corrected ephemeral Markdown reader recovered 1,210 UTF-8 bytes from the same conversation, including the final GitHub and Projects rows, without resubmission. `[HISTORICAL LIVE OBSERVATION — 2026-08-24]` Web and Deep selector errors occurred before submission on the then-tested UI. `[CURRENT IMPLEMENTATION OBSERVATION — 2026-08-28]` The Deep split lifecycle and event publication are deterministic only; the completed Deep iframe report/source extraction remains blocked on the separately approval-gated Bridge diagnostic, so #16 and live Deep usability are not complete. See `docs/M003-LIVE-RECEIPT.md` for the historical M003 receipt and `docs/M004-PLAN.md` for the current result.

## License

Original project code is licensed under [Apache-2.0](LICENSE). Existing third-party notices and license texts remain in [LICENSES](LICENSES/); the project license does not replace upstream terms.
