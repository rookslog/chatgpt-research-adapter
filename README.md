...
`status` is process-free and read-only. It validates a pre-existing completion event but does not create a missing one. `collect` invokes only `deep-research-result --wait false`; `wait` invokes only `deep-research-result --wait true`. Neither can submit a prompt. Accepted state remains resumable across restart.
